...
function insertSuggestionAndCategory(editor, suggestion, category) {
    ...
    // Get the updated text and insert the pipe category
    const text = editor.innerText;
    const lastSemicolonIndex = text.lastIndexOf(';');

    let updatedText;
    if (lastSemicolonIndex === -1) {
        updatedText = `|${category}| ${text}`;
    } else {
        updatedText = `${text.slice(0, lastSemicolonIndex + 1)} |${category}| ${text.slice(lastSemicolonIndex + 1)}`;
    }
